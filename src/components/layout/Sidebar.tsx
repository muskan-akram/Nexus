...
import { Calendar, Video, Wallet as WalletIcon } from 'lucide-react';
import { NavLink } from 'react-router-dom';
import { Home, Building2, CircleDollarSign, Users, MessageCircle, Bell, FileText, Settings, HelpCircle } from 'lucide-react';

export type UserRole = 'entrepreneur' | 'investor';

interface SidebarProps {
  role: UserRole;
  userId: string; // pass user id separately if needed
}
...
export const Sidebar: React.FC<SidebarProps> = ({ role, userId }) => {
  ...
  const investorItems = [
    ...
    { to: '/meetings', icon: <Calendar size={20} />, text: 'Meetings' },
    { to: '/video-call', icon: <Video size={20} />, text: 'Video Call' },
    { to: '/wallet', icon: <WalletIcon size={20} />, text: 'Wallet' },
  ];

  const sidebarItems = role === 'entrepreneur' ? entrepreneurItems : investorItems;
        ...
        <div className="flex-1 py-4 overflow-y-auto">
          <div className="px-3 mb-4">
            <span className="text-xs font-semibold text-gray-400 uppercase">
              {role === 'entrepreneur' ? 'Entrepreneur' : 'Investor'} Menu
            </span>
          </div>
          <div className="px-3 space-y-1">
            {sidebarItems.map((item) => (
              <SidebarItem key={item.to} to={item.to} icon={item.icon} text={item.text} />
            ))}
          </div>
